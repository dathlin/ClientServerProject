...
<strong  style="color:#ff0000">注意：在PLC的以太网模块的配置中，无法设置网络号为0，也无法设置站号为0， 所以此处均设置为1，在C#程序中也使用上述的配置，在代码中均配置为0，如果您自定义设置为网络2， 站号8，那么在代码中就要写对应的数据。如果仍然通信失败，重新测试0，0。</strong>
